...
static int input_device_evmap_init(struct input_device *device, const char *evmap_dir, const char *evmap_file)
{
    ...
    {
        errno = ENAMETOOLONG;
        syslog(LOG_ERR, "event map file directory '%s': %s\n", evmap_path, strerror(errno));
        return -1;
    }
    if (strnlen(evmap_file, PATH_MAX + 1) == PATH_MAX + 1)
    {
        errno = ENAMETOOLONG;
        syslog(LOG_ERR, "event map file name '%s': %s\n", evmap_file, strerror(errno));
        return -1;
    }

    if (strchr(evmap_file, '/'))
    {
        syslog(LOG_ERR, "event map file name '%s' contains a directory\n", evmap_file);
        return -1;
    }
    ...
    if (evmap_dir[strlen(evmap_dir) - 1] == '/')
    {
        ...
        {
            errno = ENAMETOOLONG;
            syslog(LOG_ERR, "event map file path name '%s%s': %s\n", evmap_dir, evmap_file, strerror(errno));
            return -1;
        }
    }
    else
    ...
    {
        syslog(LOG_ERR, "failed to open event map file '%s': %s\n", evmap_path, strerror(errno));
        return -1;
    }
...
int input_init(const char *evmap_dir)
{
    ...
    {
        syslog(LOG_ERR, "failed to allocate memory for the input device event map directory name %s: %s\n", evmap_dir, strerror(errno));
        input_exit();
        return -1;
    }
